...
export default function DocumentsEdit({ profile }: any) {
    ...
    return (
        <AppLayout breadcrumbs={[{ title: 'Profile', href: '/profile' }, { title: 'Edit Dokumen', href: '/profile/documents/edit' }]}>
            {/* Modal Preview Dokumen dengan Dialog */}
            <Dialog open={modalOpen} onOpenChange={setModalOpen}>
                <DialogContent className="max-w-2xl w-full">
                    <h2 className="text-lg font-semibold mb-2" id="dialog-title">Preview Dokumen</h2>
                    <DialogClose asChild>
                    ...
                    {modalFile && (modalFile.match(/\.(jpg|jpeg|png)$/i)
                        ...
                        : <iframe src={modalFile} title="Preview Dokumen" className="w-full min-h-[60vh]" aria-labelledby="dialog-title" />)}
                </DialogContent>
            </Dialog>
                                        ...
                                        {profile?.ktp_file && (
                                            <div className="mb-2">
                                                <FilePreviewButton
                                                    ...
                                                    onClick={() => { setModalFile(getDocumentFilePath(profile.ktp_file, 'ktp')); setModalOpen(true); }}
                                                >
                                        ...
                                        {profile?.ktm_or_student_card_file && (
                                            <div className="mb-2">
                                                <FilePreviewButton
                                                    ...
                                                    onClick={() => { setModalFile(getDocumentFilePath(profile.ktm_or_student_card_file, 'ktm')); setModalOpen(true); }}
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.477 0 8.268 2.943 9.542 7-1.274 4.057-5.065 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                                </FilePreviewButton>
                                            </div>
                                        ...
                                        {profile?.transcript_file && (
                                            <div className="mb-2">
                                                <FilePreviewButton
                                                    ...
                                                    onClick={() => { setModalFile(getDocumentFilePath(profile.transcript_file, 'transcript')); setModalOpen(true); }}
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.477 0 8.268 2.943 9.542 7-1.274 4.057-5.065 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                                </FilePreviewButton>
                                            </div>
